function __print_standart_functions_help() {
cat <<EOF
...
- githubremote:    Add git remote for StandartOS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the standart model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch standart_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/StandartOS-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.standart.device | grep -q "$STANDART_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $STANDART_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.standart.device | grep -q "$STANDART_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.standart.device | grep -q "$STANDART_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
