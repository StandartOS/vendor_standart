...
func init() {
	android.RegisterModuleType("standart_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = standartExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = standartExpandVariables(ctx, dep_file)
	...
	cmd := standartExpandVariables(ctx, String(g.properties.Cmd))
